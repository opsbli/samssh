...
## 架构
Java 21, Spring Boot 3.x, MyBatis-Plus, Sa-Token, MySQL 8.0, Redis, Quartz, Docker
...
- **下游影响扫描** — spec-discovery must trace 1-2 levels downstream (using `codegraph_explore` / callers / callees) for each evidence gap to identify cascading fix points before closing the discovery phase. Findings must be recorded in the Evidence or noted as potential follow-up items.
...
## Design Rules
- `db_schema.sql` is database truth.
- `api_swagger.yaml` is API truth.
- `domain_model.puml` is domain truth.
...
## Review Rules
- **对称性检查** — When modifying one side of a symmetric pair (checkin/checkout, create/delete, submit/approve, enable/disable), the review must audit the counterpart for the same class of issue. If found, either fix it in the same change or record a follow-up Decision.

---
- **Primary tool**: Use `codegraph_explore` MCP tool (from the codegraph MCP server) as the first choice for ALL code lookup needs — symbol search, architecture questions, "how does X work?", call paths, impact analysis, and "where is X defined?"
- **How to use**: Give it a natural-language question or a bag of symbol/file names. It returns verbatim source grouped by file, call paths between symbols (including dynamic-dispatch hops), and a blast-radius summary — all in one call.
- **When to fall back**: Only fall back to grep/Read/lsp tools when `codegraph_explore` returns no useful results (e.g., the codebase is not indexed or the query type is unsupported).
- **Available tools**: `codegraph_explore` (primary), `codegraph_node` (one symbol source + caller/callee trail), `codegraph_search` (symbol search by name), `codegraph_callers` (find callers), `codegraph_callees` (find callees), `codegraph_impact` (blast radius), `codegraph_files` (file structure), `codegraph_status` (index health)
...
- **核心方法测试覆盖** — When modifying a method that has zero direct test coverage, the implement phase MUST include adding or updating unit tests covering its boundary conditions. This gate overrides the "微修复路径" skip-test exemption when the method in question has no coverage.
...
[Phase 4 — 设计]  spec-design
  │  Update db_schema / api_swagger / domain_model
  ▼
...
```
> 跳过的 phase（grill/sync/trace）必须有门禁豁免理由：grill 可跳过当原则已在前序 Decision 中确立或用户已直接确认；sync 可跳过当变更不涉及新证据/决策/需求文件；trace 可跳过当变更仅涉及已有追溯链的末端扩展。以上豁免理由须在 spec-confirm 的 Decision 记录中注明。
...
- **Phase 6→7**: 代码与设计一致
- **Phase 6→7 (增强门禁)**: 若被修改的方法零测试覆盖，testing phase 必须附带新增边界条件测试（此增强门禁覆盖微修复路径的 test 豁免）
- **Phase 7→8**: 测试通过
- **Phase 8→9**: 全链路无缺失链接
